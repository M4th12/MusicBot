...
/**
 * Plays a track or URL, using LavaSrc for Spotify searches.
 */
public class PlayCmd extends MusicCommand {
    ...
    public PlayCmd(Bot bot) {
        ...
        this.children = new MusicCommand[]{ new PlaylistCmd(bot) };
    }
        ...
        if(event.getArgs().isEmpty() && event.getMessage().getAttachments().isEmpty()) {
            ...
            for(Command cmd: children) {
                builder.append("\n`")
                       ...
                       .append(cmd.getName()).append(" ").append(cmd.getArguments())
                       .append("` - ").append(cmd.getHelp());
            }
            event.reply(builder.toString());
            return;
        }

        // parse args or attachment URL
        String args = event.getArgs().isEmpty()
            ? event.getMessage().getAttachments().get(0).getUrl()
            : (event.getArgs().startsWith("<") && event.getArgs().endsWith(">")
                ? event.getArgs().substring(1, event.getArgs().length()-1)
                : event.getArgs());

        // Determine identifier: URL versus Spotify search
        String identifier = args.startsWith("https")
            ? args
            : "spsearch:" + args;

        // Load via playerManager
        event.reply(loadingEmoji + " Loading... `[" + args + "]`", m -> 
            bot.getPlayerManager().loadItemOrdered(
                event.getGuild(),
                identifier,
    ...
    private class ResultHandler implements AudioLoadResultHandler {
        ...
        private void loadSingle(AudioTrack track, AudioPlaylist playlist) {
            ...
            String addMsg = FormatUtil.filter(event.getClient().getSuccess()
                + " Added **" + track.getInfo().title + "** (`" + TimeUtil.formatTime(track.getDuration()) + "`) "
            ...
            if(playlist == null || !event.getSelfMember().hasPermission(event.getTextChannel(), Permission.MESSAGE_ADD_REACTION)) {
                ...
                new ButtonMenu.Builder()
                    ...
                    .setFinalAction(msg -> { try { msg.clearReactions().queue(); } catch(PermissionException ignore) {} })
                    .build().display(m);
            }
        }
        ...
        @Override public void trackLoaded(AudioTrack track) { loadSingle(track, null); }
        @Override public void playlistLoaded(AudioPlaylist playlist) {
            if(playlist.getTracks().size()==1 || playlist.isSearchResult()) {
                AudioTrack single = playlist.getSelectedTrack()!=null ? playlist.getSelectedTrack() : playlist.getTracks().get(0);
                ...
                if(playlist.getTracks().isEmpty()) {
                    m.editMessage(FormatUtil.filter(event.getClient().getWarning()
                        + " The playlist could not be loaded or contained 0 entries")).queue();
                } else if(count==0) {
                    m.editMessage(FormatUtil.filter(event.getClient().getWarning()
                        + " All entries in this playlist were longer than the allowed maximum (`"
                        + bot.getConfig().getMaxTime() + "`)")).queue();
                } else {
                    m.editMessage(FormatUtil.filter(event.getClient().getSuccess()
                        + " Found playlist with `" + playlist.getTracks().size() + "` entries; added to the queue!"
                        + (count<playlist.getTracks().size() ? "\n" + event.getClient().getWarning()
                        + " Tracks longer than the allowed maximum (`" + bot.getConfig().getMaxTime() + "`) have been omitted." : ""))).queue();
                }
            }
        }
